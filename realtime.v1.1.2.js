...
(async function(){
  ...
  function processAiEvent(msg) {
    try {
      console.log('Processing AI event:', msg.type, msg);
      
      // Handle AI response text/audio transcription
      if (msg.type === 'response.audio_transcript.delta') {
        ...
        upsertBubble(msg.item_id, 'agent', msg.transcript || '', true);
      }
      else if (msg.type === 'response.text.done') {
        upsertBubble(msg.item_id, 'agent', msg.text || '', true);
      }
      else if (msg.type === 'response.output_item.done') {
        // Handle completed response items
        if (msg.item && msg.item.content) {
          for (const content of msg.item.content) {
            if (content.type === 'text') {
              upsertBubble(msg.item.id, 'agent', content.text || '', true);
            } else if (content.type === 'audio' && content.transcript) {
              upsertBubble(msg.item.id, 'agent', content.transcript, true);
            }
          }
        }
      }
        ...
        updateStatus('🎤 檢測到語音輸入...', 'connected');
      }
      else if (msg.type === 'input_audio_buffer.speech_stopped') {
        updateStatus('🔄 正在處理語音...', 'connected');
      }
    } catch (error) {
    ...
    try {
      ...
      dc.onmessage = (e) => {
        try {
          ...
          // Handle user transcription properly
          if (msg.type === 'conversation.item.input_audio_transcription.delta') {
            const prev = bubbles[msg.item_id]?.querySelector('.text').textContent || '';
            // Filter out audio tokens and only show actual transcribed text
            const cleanDelta = (msg.delta || '').replace(/<\|[^|]*\|>/g, '');
            if (cleanDelta.trim()) {
              upsertBubble(msg.item_id, 'user', prev + cleanDelta, false);
            }
          }
          else if (msg.type === 'conversation.item.input_audio_transcription.completed') {
            // Clean the transcript of any audio tokens
            const cleanTranscript = (msg.transcript || '').replace(/<\|[^|]*\|>/g, '').trim();
            if (cleanTranscript) {
              upsertBubble(msg.item_id, 'user', cleanTranscript, true);
            } else {
              // If no clean transcript, show a placeholder
              upsertBubble(msg.item_id, 'user', '[語音輸入]', true);
            }
          }
            ...
            if (content?.type === 'input_audio' && content.transcript) {
              const cleanTranscript = content.transcript.replace(/<\|[^|]*\|>/g, '').trim();
              if (cleanTranscript) {
                upsertBubble(msg.item.id, 'user', cleanTranscript, true);
              }
            }
          }
          else {
